...
const AddFoto = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await Swal.fire({
      title: "Simpan Data?",
      text: "Apakah Anda yakin ingin menyimpan data Galeri Video ini?",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        data.append("titlevideo", formData.titlevideo);
        data.append("kategori", formData.kategori);
        data.append("urlvideo", formData.urlvideo);
        if (formData.image) data.append("image", formData.image); // Tambahkan file gambar jika ada

        const response = await axiosInstance.patch(`/galerivideo/${newsToEdit.id}`, data, {
          headers: {
        ...
        if (response.status === 201) {
          Swal.fire("Data Galeri Video telah disimpan!", "", "success");
          navigate("/video"); 
        } else {
