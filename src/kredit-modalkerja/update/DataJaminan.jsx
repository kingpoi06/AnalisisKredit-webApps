...
export default function UpdateDataJaminan() {
  ...
  const normalizeJaminanItem = (item) => {
    ...
    Object.entries(item || {}).forEach(([rawKey, value]) => {
      ...
      if (
        !mapped.slikFileName &&
        typeof value === "string" &&
        normalizedKey.includes("slik")
      ) {
        const trimmedValue = value.trim();
        if (trimmedValue) {
          mapped.slikFileName = trimmedValue;
        }
      }
      const targetKey = JAMINAN_FIELD_MAP[normalizedKey];
      if (!targetKey) return;
      if (
        targetKey === "slik" ||
        targetKey === "slikTable" ||
        targetKey === "dokumentasiAgunan"
      ) {
        return;
      }
    ...
    if (mapped.jenisjaminan === "BPKB") {
      ...
      if (
        ...
        mapped.nilaiLikuidasi === undefined
      ) {
    ...
    if (mapped.jenisjaminan === "Sertifikat") {
      const totalNjop = getTotalNilaiNJOP(mapped);
      const rerataPasar = getNilaiRerataPasar(mapped);
      const rerataValue = rerataPasar.hasValue ? rerataPasar.total : "";
      const margin = getSertifikatMargin(mapped.statusPengikatan);
      if (totalNjop.hasValue) {
        mapped.nilaiNJOP = String(totalNjop.total);
      }
      mapped.safetyMargin = String(Math.round(margin * 100));
      if (rerataValue !== "") {
        mapped.rerataNilaiPasar = String(rerataValue);
        mapped.taksiranPasar = String(rerataValue);
        mapped.nilaiLikuidasi = computeNilaiLikuidasi(rerataValue, margin);
      }
    }

    if (mapped.jenisjaminan === "Deposito" && !mapped.buktiHakMilik) {
      mapped.buktiHakMilik = "Bilyet Deposito";
    }
  ...
  const applyJaminanList = (items) => {
    if (!items.length) return;
    const mapped = items.map(normalizeJaminanItem).slice(0, 3);
    const nextTotal = Math.max(1, mapped.length);
    setTotalJaminan(nextTotal);
  ...
  const fetchDataJaminan = async (requestNo) => {
    ...
    try {
      ...
      applyJaminanList(filtered.length ? filtered : list);
    } catch (error) {
      try {
        const response = await axios.get(
  ...
  useEffect(() => {
    ...
    try {
      ...
      fetchDataJaminan(no_permohonan);
    } catch {
      localStorage.removeItem("accessToken");
      navigate("/");
    }
  ...
  useEffect(() => {
    setJaminanList((prev) => {
      ...
      const next = prev.map((item) => {
        ...
        if (String(item.plafonDiajukan ?? "") === String(nextPlafon)) {
          return item;
        }
        changed = true;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const item of jaminanList) {
        ...
        await axios.patch(
          API_ENDPOINTS.datanasabah.dataJaminan.detail(no_permohonan),
          payload,
          {
            headers: {
      ...
      Swal.fire("Berhasil", "Data Jaminan berhasil diperbaharui", "success");
      navigate("/dashboard");
    } catch (err) {
      Swal.fire(
        "Gagal",
                      ...
                      {item.jenisjaminan === "Sertifikat" && (
                        <div className="space-y-4">
                          ...
                          {showNilaiAgunanDetails[index] ? (
                            <div className="space-y-4 rounded-xl border border-slate-100 bg-slate-50/70 p-4">
                              <Input
                                label="Harga Taksiran dari Desa / Kelurahan"
                                type="text"
                                value={formatIdInteger(
                                  item.nilaiTaksiranKelurahan
                                )}
                                onChange={handleItemNumberChange(
                                  index,
                                  "nilaiTaksiranKelurahan"
                          ...
                          <Input
                            ...
                            value={computeMinimalHakTanggungan(plafonValue)}
                            readOnly
                          />
                          {/* <Input
                            label="Nilai Pengikatan Jaminan"
                            placeholder="Diisi jika ada pengikatan, nilai pengikatan = taksiran harga bank"
                            value={item.pengikatanJaminan}
