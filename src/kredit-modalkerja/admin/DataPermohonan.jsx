...
const isKreditKonsumtif = (value) => {
  if (!value) return false;
  const normalized = resolveJenisKreditLabel(value).toLowerCase();
  if (normalized.includes("konsumtif")) return true;
...
const fetchJenisKreditFromGenerate = async (requestNo) => {
  ...
  try {
    ...
    setFormData((prev) =>
      prev.jenisKredit === resolved ? prev : { ...prev, jenisKredit: resolved }
    );
  } catch (err) {
    Swal.fire(
      "Gagal",
...
const handleSave = async () => {
  ...
  const { name: pengusulName, role: userRole } = getUserFromToken();
  ...
  if (userRole === "officer" && pengusulName) {
    payload.append("namaPengusul", pengusulName);
  }
  ...
  try {
    ...
    await axios.post(
      API_ENDPOINTS.datanasabah.dataPermohonan.list(),
      payload,
      {
        headers: {
    ...
    Swal.fire("Berhasil", "Data Permohonan Kredit berhasil disimpan", "success");

    // ⬇️ lanjut ke jaminan (pakai nik yg sama)
    const nextPath = isKreditKonsumtif(formData.jenisKredit)
      ? `/master-data/data-instansi/${encodeURIComponent(requestNoPermohonan)}`
      : `/master-data/data-usaha/${encodeURIComponent(requestNoPermohonan)}`;
    navigate(nextPath);
